...
# Leechy-bot Telegram
So basically Leechy-bot is aimed to be the most versatile torrent leecher and Youtube-DL bot for telegram. This bot is highly customizable and to customize this bot you don't need to restart the bot every time. 
...
Table of Content
- [FEATURES](#features)
- [TEST THE BOT (DEMO)](https://t.me/multi_purpose_bot)
...
- If the bot is in a group, the users have their own settings like:
...
- Extraction of ZIP, TAR, ISO, RAR wih and without password. If you chose to extract the archive and you enter a wrong password, it will prompt you to enter the password upto 3 times after that zip will be uploaded as it is.
...
1. Run the following commands. (Following commands can be used to setup the vps from scratch)
    ...
    1. `git clone https://github.com/EDM115/leechy-bot.git`
    2. `sudo apt update`
...
4. And finally run this in cloned folder.
...
- `CENTRAL_UPDATE`
  ...
  - Default Value :- `True`
  - Use :- All the tasks will be updated in one single message rather than each separate.
