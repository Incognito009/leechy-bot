...
async def handle_ytdl_callbacks(e: MessageLike):
    ...
    if data[0] == "ytdlsmenu":
        if data[2] != str(e.sender_id):
            await e.answer("Not valid user, Don't touch 😡")
            return
...
async def handle_ytdl_playlist(e: MessageLike) -> None:
    ...
    try:
        ...
        if entlen > get_val("MAX_YTPLAYLIST_SIZE"):
            ...
            await msg.edit(f"Playlist too large max {get_val('MAX_YTPLAYLIST_SIZE')} vids allowed as of now. This one has {entlen}")
            return
...
async def get_ytdl_choice(e,timestamp):
    ...
    while not lis[0]:
        if (time.time() - start) >= 300: #TIMEOUT_SEC:
...
class YTDLController:
    ...
    async def execute(self):
        ...
        if data[2] != str(self.callback.sender_id):
            await self.callback.answer("Not valid user, Don't touch 😡")
            return False
        else:
        ...
        if os.path.exists(path):
            ...
            if self._quality.endswith("k"):
                ...
                if err and ofiles < 2 :
                    ...
                    if err:
                        self._is_errored = True
                        self._error_reason = "Some videos from this have errored in download (i.e private or deleted videos in playlist) Uploading which are successfull."
