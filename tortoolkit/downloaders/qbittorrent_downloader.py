...
class QbittorrentDownloader(BaseTask):
    ...
    async def resume_all(self):
        ...
        msg += "▶️ Resumed {} torrents check the status for more… ▶️".format(len(tors))
        ...
        while True:
            ...
            try:
                ...
                if (is_meta and (time.time() - meta_time) > get_val("TOR_MAX_TOUT")):
                    ...
                    torlog.error("This torrent has no seeds 😪 clearing that torrent now. Torrent : {} - {}".format(tor_info.hash,tor_info.name))
                    await self.delete_this(tor_info.hash)
                ...
                try:
                    if tor_info.state == "error":
                        ...
                        torlog.error("This torrent has error 💀 clearing that torrent now. Torrent : {} - {}".format(tor_info.hash,tor_info.name))
                        await self.delete_this(tor_info.hash)
