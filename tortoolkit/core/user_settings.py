...
tordb = TorToolkitDB()
header =  '<b>**MultiPurpose Bot** by <a href="https://github.com/EDM115">@EDM115</a></b>\n<u>USER SETTINGS MENU - v1</u>'
...
async def general_input_manager(e,mmes,var_name,datatype,value,sender_id,sub_menu):
    if value is not None and not "ignore" in value:
        ...
        if conf is not None:
            ...
            await handle_user_settings(mmes,True,f"<b><u>Confirm timed out [waited 300s for input].</b></u>",sub_menu, sender_id=sender_id)
    else:
        await handle_user_settings(mmes,True,f"<b><u>Entry Timed out [waited 300s for input]. OR else ignored.</b></u>",sub_menu, sender_id=sender_id)
