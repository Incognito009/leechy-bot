...
TIMEOUT_SEC = 300
#5 min before automatically use default task (made for poor connection users)

# this file will contain all the handlers and code for settings
...
tordb = TorToolkitDB()
header =  '<b>**MultiPurpose Bot** by <a href="https://github.com/EDM115/leechy-bot">EDM115</a></b>\n<u>ADMIN SETTINGS MENU - Beta v1</u>'
async def handle_setting_callback(e):
    ...
    if cmd[1] == "fdocs":
        ...
        await e.answer("Type the new value for completed progress string. Note that only one character is expected",alert=True)
...
async def handle_settings(e,edit=False,msg="",submenu=None,session_id=None):
    ...
    if submenu is None:
        ...
        await get_bool_variable("USETTINGS_IN_PRIVATE","User settings in private",menu,"allowuset",session_id)
...
async def general_input_manager(e,mmes,var_name,datatype,value,db,sub_menu):
    if value is not None and not "ignore" in value:
        ...
        if conf is not None:
            ...
            await handle_settings(mmes,True,f"<b><u>Confirm timed out [waited 300s for input]</b></u>",sub_menu)
    else:
        await handle_settings(mmes,True,f"<b><u>Entry Timed out [waited 300s for input] OR else ignored</b></u>",sub_menu)
