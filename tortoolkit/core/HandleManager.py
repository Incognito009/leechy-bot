...
async def handle_settings_cb(e):
    if await is_admin(e.client,e.sender_id,e.chat_id):
        ...
        await e.answer("⚠️ WARN ⚠️ Don't touch admin settings",alert=True)
...
async def start_handler(event):
    msg = "<b>Hello, this is MultiPurpose Bot an instance of</b> <a href='https://github.com/EDM115/leechy-bot'>this repo</a>. <i>s-sorry for no help now… 🥺</i>"
...
async def set_thumb_cmd(e):
    ...
    try:
        ...
        await e.reply("Error while processing the custom thumbnail. Retry again")
        return
...
def term_handler(signum, frame, client):
    ...
    async def term_async():
        omess = None
        #st = Status().Tasks
        msg = "Bot is rebooting and re-adding your tasks…\n\n"
        for i in st:
...
async def booted(client):
    ...
    for i in chats:
        try:
            await client.send_message(i, "The bot is booted and ready to use 🥰")
        except Exception as e:
