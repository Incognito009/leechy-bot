...
class RcloneUploader(BaseTask):
    ...
    async def execute(self):
        ...
        if not os.path.exists(path):
            torlog.info(f"Returning none because the path {path} not found")
            self._is_errored = True
            self._error_reason = f"Returning none because the path {path} not found"
            return False
...
class RcloneController:
    ...
    async def execute(self):
        ...
        if self._rclone_up.is_errored:
            ...
            buttons = [
                [KeyboardButtonUrl("☁️ Drive URL", drive_link)]
            ]
